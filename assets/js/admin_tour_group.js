...
$(function() {
    ...
    $(".btn-dialog-save-itinerary").on("click", function() {
        ...
        if (date == "") {
            ...
            var currentDateString = date.split("-");
            var currentDateItem = new Date(parseInt(currentDateString[2]), parseInt(currentDateString[1]) - 1, parseInt(currentDateString[0]));
            var day = weekday[currentDateItem.getDay()];
            ...
            element += "<div class='itinerary-hidden-item-date'>" + day + ", " + date + "</div>";
            ...
            if (done) {
                ...
                $(".itinerary-hidden-item").each(function() {
                    var dateString = $(this).find("input[name='itinerary_date']").val().split("-");
                    var dateItem = new Date(parseInt(dateString[2]), parseInt(dateString[1]) - 1, parseInt(dateString[0]));

                    if (currentDateItem > dateItem) {
                        afterElement = $(this);
                    } else {
                        return false;
                    }
